...
if __name__ == "__main__":
    ...
    root = "profile_"
    dir_files = "/scisoft/users/srio/MLCRL/V25/sampled_profiles/" # profiles
    dir_wofrydata = "/scisoft/users/srio/MLCRL/V25/wofry_results/" # wofry simulations
    dir_out = "/scisoft/users/srio/MLCRL/V25/"  # CNN data
    only1000 = False
    model_root = "training_v25_n1000_epoch50"
    basis_used = 'gs'
    nbin = 1
    pstart = 100
    n_files = 1000
    ...
    if True:
        ...
        try:
            ...
            print(">>>> Fail to load %s/%s.json  TRY ...checkpoint.log" % (dir_out, model_root))

            # import pandas as pd
            #
            # log_data = pd.read_csv("%s/%s_checkpoint.log" % (dir_out, model_root),
            #                        sep=',',
            #                        engine='python')
            log_data = numpy.loadtxt("%s/%s_checkpoint.log" % (dir_out, model_root), skiprows=1, delimiter=',')
